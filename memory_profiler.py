...
def plot_memory_profiles(
    profiles: List[MemoryProfile], title: str = "Memory Usage Comparison"
) -> None:
    """
    ...
    for profile in profiles:
        plt.plot(
            ...
            label=f"{profile.name} (Peak: {profile.peak_memory:.2f} MB)",
        )
    ...
    peak_text = "\n".join(
        [
            f"{p.name}: {p.peak_memory:.2f} MB (Time: {p.execution_time:.2f}s)"
            for p in profiles
        ]
    )
    plt.figtext(
